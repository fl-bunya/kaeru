...
async function getCurrentTimeParams(): Promise<{ y: string; m: string; d: string; hh: string; m1: string; m2: string }> {
  ...
  // console.log('%cindex.ts:31 now', 'color: #007acc;', year, month, day, hour, minute);
...
async function fetchTransitRoutes(): Promise<RouteInfo[]> {
  try {
    ...
    // デバッグ: HTML構造を詳しく調べる
    // console.log('HTML構造を分析中...');
    // console.log('Title:', $('title').text());
    // const firstRoute = $('.routeDetail').first();
    // if (firstRoute.length > 0) {
    //   console.log('First route HTML structure:');
    //   console.log(firstRoute.html()?.substring(0, 500));
    // }
...
async function main() {
  ...
  routes.forEach((route, index) => {
    ...
    // console.log(`所要時間: ${route.duration || 'N/A'}`);
    // console.log(`乗換回数: ${route.transfers || 'N/A'}`);
    // console.log(`運賃: ${route.fare || 'N/A'}`);
